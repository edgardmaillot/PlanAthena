...
{
    public static class ProjectDataLoader
    {
        // Votre méthode CreateOperation, nécessaire pour que le code compile
        private static OperationInput CreateOperation(string operationId, string tradeCode, int hours, int maxWorkersInBloc, string dependencies = "", int maxWorkersOnOp = 0)
        {
            return new OperationInput
            {
                ...
                TradeCode = tradeCode,
                Hours = hours,
                DependsOnOperationIds = dependencies.Split(new[] { ',', ' ' }, System.StringSplitOptions.RemoveEmptyEntries).ToList(),
                MaxConcurrentWorkersOnOperation = maxWorkersOnOp > 0 ? maxWorkersOnOp : maxWorkersInBloc
            };
        }

        // VOTRE MÉTHODE, intégrée ici
    ...
    public class PlanningRequestInput
    {
        public string? ChantierId { get; set; }
        public List<BlocInput> Blocs { get; set; } = new List<BlocInput>();
        public List<ZoneLogicInput> Zones { get; set; } = new List<ZoneLogicInput>();
        public List<WorkerInput> Workers { get; set; } = new List<WorkerInput>();
        public List<TradeDefinition> TradeDefinitions { get; set; } = new List<TradeDefinition>();
    }

    public class BlocInput
    {
        public string BlocId { get; set; } = string.Empty;
        public int MaxConcurrentWorkersInBloc { get; set; } = 1;
        public string? AssociatedZoneId { get; set; }
        /// <summary>
        /// Liste des opérations granulaires à réaliser dans ce bloc.
        /// Chaque OperationInput deviendra une ChantierTask distincte.
        /// </summary>
        public List<OperationInput> Operations { get; set; } = new List<OperationInput>();
    }

    public class ZoneLogicInput
    {
        public string ZoneId { get; set; } = string.Empty;
        public int PriorityLevel { get; set; } = 3;
        public List<string> DependsOnZoneIds { get; set; } = new List<string>();
    }
    ...
    public class TradeDefinition
    {
        ...
        /// <summary>
        /// Définit les dépendances métier. Si une Opération du TradeCode X dépend
        /// de ce TradeCode, elle devra attendre que TOUTES les opérations de
        /// ces TradeCodes prérequis soient terminées DANS LE MÊME BLOC.
        /// </summary>
        public List<string> DependsOnTradeCodes { get; set; } = new List<string>();
    }
    ...
    public class WorkerInput
    {
        ...
        public List<string> SkillTradeCodes { get; set; } = new List<string>();
    }

    /// <summary>
    /// Représente une opération ou une tâche fine avec sa propre identité et ses dépendances.
    /// Anciennement TradeHoursInput.
    /// </summary>
    public class OperationInput
    {
        /// <summary>
        /// Identifiant unique de cette opération (ex: "SDB_DEPOSE_B", "CUISINE_POSE_EVIER").
        /// Doit être unique au sein de son BlocInput parent.
        /// </summary>
        public string OperationId { get; set; } = string.Empty;
        public string TradeCode { get; set; } = string.Empty;
        public double Hours { get; set; }
        /// <summary>
        /// Liste des OperationId (au sein du même BlocInput) dont cette opération dépend directement.
        /// </summary>
        public List<string> DependsOnOperationIds { get; set; } = new List<string>();
        public int MaxConcurrentWorkersOnOperation { get; set; } = 1; // Limite spécifique à cette opération
    }

    // PlanificationResult et WorkerPerformanceMetrics sont dans PlanificationResults.cs
    public class ScheduledTaskEntry
    {
        ...
        public string WorkerIdDebug { get; set; } = string.Empty;
        public string TaskIdDebug { get; set; } = string.Empty; // Sera maintenant BlocId + OperationId
    }
